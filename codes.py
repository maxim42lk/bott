...
keyboard_test.row("Привет", "Пока", "Словарь для программиста", "Кибербезопасность", "Для IT и бизнеса")
...
def get_age(message):
    ...
    a.send_message(message.from_user.id, 'Тебе ' + str(age) + ' лет, тебя зовут ' + name + ' ' + surname + '?')
...
def sendYourMessage(message):
    ...
    if message.text == "Привет":
        ...
        a.send_message(mid,
                       "DARKReading.com — известный сайт, который полностью посвящен особенной теме — кибербезопасности")
    elif message.text == "Для IT и бизнеса":
        a.send_message(mid,
                       "CIO.com — отличный американский ресурс с обилием контента на многие темы как для IT-специалистов, так и для руководящего звена IT-компаний")
    elif message.text == "Словарь для программиста":
        a.send_message(mid,
                       "multitran.ru — отличный словарь с кучей значений на каждое слово, есть категории применения слов и подробные пояснения;")
